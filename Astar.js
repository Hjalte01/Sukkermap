    ...
    loop() {
        if (
            ...
            if (setupBol) {
                ...
                for (let i = 0; i < arrayTemp.length; i++) {
                    if (arrayTemp[i].text != undefined) {
                        if (arrayTemp[i].text == inputStart)
                            start = arrayTemp[i];
            ...
            if (openSet.length > 0) {
                ...
                console.log(
                    "fejl, noget gik galt, formodentlig en connection error"
                );
                alert(
                    "Hjælp os med at skrive en bugRapport for en bedre version, gerne skriv hvilke to steder i skrev i søgefeltet for at fejlen forekommer :)"
                );
                setupBol = true;
                inputEnd = undefined;
                inputStart = undefined;
            ...
            if (start != undefined)
                if (
                    start.tag == `${planTegning}. sal` ||
                    (start.tag == "stue" && planTegning == 0) ||
                    planTegning == undefined
                )
                    text(start.text, start.x, start.y);
            if (end != undefined)
                if (
                    end.tag == `${planTegning}. sal` ||
                    (end.tag == "stue" && planTegning == 0) ||
                    planTegning == undefined
                )
            ...
            for (let i = 0; i < path.length; i++) {
                if (
                    path[i].tag == `${planTegning}. sal` ||
                    (path[i].tag == "stue" && planTegning == 0)
                ) {
                    vertex(path[i].x, path[i].y);
                    if (
                        (i < path.length - 1 &&
                            path[i].tag == `${planTegning}. sal` &&
                            path[i + 1].tag != `${planTegning}. sal`) ||
                        (i < path.length - 1 &&
                            path[i].tag == "stue" &&
                            path[i + 1].tag != "stue")
                    ) {
            ...
            if (keyIsDown(107)) {
                ...
                var tempPrompt = prompt(
                    'arrayTemp[100] = new spot(879, 474, "tag", "sal");'
                );
                console.log(
                    "arrayTemp[" +
                        arrayTemp.length +
                ...
                arrayTemp[arrayTemp.length] = new spot(
                    round(mouseX, 0),
                    round(mouseY, 0),
                    tempPrompt
                );
            ...
            if (keyIsDown(110) || keyKomma) {
                //key "," for numlock for index
                keyKomma = true;
                push();
                textSize(15);
                noStroke();
            ...
            if (keyIsDown(109)) {
                //key - for numlock for nabo
                var tempPrompt = prompt(
                    "Indtast naboer til index: " +
                        [arrayTemp.length - 1] +
                        ". Skriv f.eks: 1, 2, 4"
                );
                console.log(
    ...
    if (triCount.fejl == false) {
        if (triCount.change == true) {
            ...
            if (planTegning == undefined) {
                ...
                for (let i = 0; i < path.length; i++) {
                    if (
                        path[ceil(i)].tag == `${planTegning}. sal` ||
                        (path[ceil(i)].tag == "stue" && planTegning == 0)
                    ) {
                        if (triCount.change == true) {
                            triCount.mindsteVærdi[
                                triCount.mindsteVærdi.length
                            ] = i;
        ...
        if (triCount.mindsteVærdi != "" || triCount.maxVærdi != "") {
            ...
            triCount.maxVærdi[-1] =
                triCount.maxVærdi[triCount.mindsteVærdi.length - 1]; // lidt sketchy måde at lave arr[-1]
            for (let i = triCount.mindsteVærdi.length - 1; i >= 0; i--) {
                if (
                    (ceil(triCount.pivot) <= triCount.mindsteVærdi[0] &&
                        i == 0) ||
                    (ceil(triCount.pivot) <= triCount.mindsteVærdi[i] &&
                        ceil(triCount.pivot) > triCount.maxVærdi[i - 1])
                ) {
                    triCount.pivot = triCount.maxVærdi[i - 1];

                    if (i == 0) {
                        triCount.pivot =
                            triCount.maxVærdi[triCount.mindsteVærdi.length - 1];
                    }
                }
            }
            ...
            let tempPoint2Rate =
                10 / sqrt(tempPoint1_2.x ** 2 + tempPoint1_2.y ** 2);
            if (
                path[ceil(triCount.pivot - 1)].tag == `${planTegning}. sal` ||
                planTegning == undefined ||
                (path[ceil(triCount.pivot - 1)].tag == "stue" &&
                    planTegning == 0)
            ) {
                triangle(
                    path[ceil(triCount.pivot)].x -
                        tempPoint1_2.y * tempPoint2Rate,
                    path[ceil(triCount.pivot)].y +
                        tempPoint1_2.x * tempPoint2Rate,
                    path[ceil(triCount.pivot)].x +
                        tempPoint1_2.x * tempPoint2Rate,
                    path[ceil(triCount.pivot)].y +
                        tempPoint1_2.y * tempPoint2Rate,
